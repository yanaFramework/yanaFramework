<?php
/**
 * YANA_Sniffs_Whitespace_ScopeIndentSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class YANA_Sniffs_WhiteSpace_ScopeIndentSniff extends Generic_Sniffs_WhiteSpace_ScopeIndentSniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = ($scopeOpener + 1); $i < $scopeCloser; $i++) {
            ...
            if ($newline === true && $tokens[$i]['code'] !== T_WHITESPACE) {
                ...
                } else if ($tokens[$firstToken]['code'] === T_DOC_COMMENT) {
                ...
                if ($column !== $currentIndent) {
                    if ($this->exact === true || $column < $currentIndent) {
                        $error  = 'Line indented incorrectly; expected ';
                        if ($this->exact === false) {
                            $error .= 'at least ';
                        }

                        $error .= ($currentIndent - 1).' spaces, found ';
                        $error .= ($column - 1);
