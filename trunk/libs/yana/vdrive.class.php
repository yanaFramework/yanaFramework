...
class VDrive extends FileSystemResource implements \Yana\Report\IsReportable, \Serializable
{
    ...
    /** @var VDriveConfiguration */ private $content = null;
    /**#@-*/
    ...
    public function read()
    {
        ...
        $this->content = VDriveConfiguration::loadString($content);
        /* read XML */
        if (!($this->content instanceOf VDriveConfiguration)) {
     ...
     *
     * @access  private
     * @param   VDriveConfiguration  $content  current xml node
     * @param   string               $path     current virtual path
     */
    private function _readXML(VDriveConfiguration $content, $path = "")
    {
        ...
        foreach ($content as $node)
        {
            if ($node->isMountpoint()) {
                ...
                if ($node->isDir()) {
                    ...
                    $this->drive[$name] = new VDriveDir($source);
     ...
     *
     * @access  private
     * @param   VDriveConfiguration  $content   content
     * @return  string
     */
    private function _getSource(VDriveConfiguration $content)
    {
