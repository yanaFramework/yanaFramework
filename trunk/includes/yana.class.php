...
final class Yana extends Singleton implements IsReportable
{
    ...
    private function _getRegistry()
    {
        if (!isset($this->registry)) {
            ...
            if (YANA_CACHE_ACTIVE === true && file_exists($cacheFile)) {
                $this->registry = Registry::unserialize(file_get_contents($cacheFile));
            } else {
            ...
            // create cache file
            if (YANA_CACHE_ACTIVE === true && !file_exists($cacheFile)) {
                file_put_contents($cacheFile, $this->registry->serialize());
            }
    ...
    private function _getPlugins()
    {
        if (!isset($this->plugins)) {
            ...
            if (YANA_CACHE_ACTIVE === true && file_exists($cacheFile)) {
                $this->plugins = PluginManager::unserialize(file_get_contents($cacheFile));

            } else {
                $this->plugins = PluginManager::getInstance();
                file_put_contents($cacheFile, $this->plugins->serialize());
            }
        }
        return $this->plugins;
    }
    ...
    private function _getSkin()
    {
        if (!isset($this->skin)) {
            ...
            if (YANA_CACHE_ACTIVE === true && $cacheFile->exists()) {
                ...
                $this->skin = Skin::unserialize(file_get_contents($cacheFile->getPath()));
                ...
                if (YANA_CACHE_ACTIVE === true) {
                    $cacheFile->create();
                    $cacheFile->setContent($this->skin->serialize());
                    $cacheFile->write();
                }
            }
        }
    ...
    public function getReport(ReportXML $report = null)
    {
        ...
        unset($i);
        $root = dirlist("./includes/", '.php');
        assert('!isset($i); /* Cannot redeclare variable $i */');
        for ($i = 0; $i < count($root); $i++)
        {
            $subreport->addText("includes/{$root[$i]} = " . md5_file("includes/{$root[$i]}"));
        }
        unset($i);
