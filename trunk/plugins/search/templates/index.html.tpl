...
//--></script>
  ...
  <table cellspacing="0" cellpadding="8" id="statistics" align="center" summary="{lang id="STATS_NAME"}">
   ...
   <!-- {assign var="i" value=$i+1} -->
     ...
     <!-- {/if} -->
       <th align="left"  class="search_label"><!-- {if $CURRENT.SEARCHCOUNT != $last_count} -->{$i}<!-- {/if} --></th>
       <td align="right" class="description">{$CURRENT.SEARCHTERM}</td>
       <td align="right" class="comment">{$CURRENT.SEARCHCOUNT}&nbsp;<!-- {if $CURRENT.SEARCHCOUNT > 1} -->{lang id="STATS_HITS"}<!-- {else} -->{lang id="STATS_HIT"}<!-- {/if} --></td>
     </tr>
   <!-- {assign var="last_count" value=$CURRENT.SEARCHCOUNT} -->
   {foreachelse}
   <tr>
     <td>
