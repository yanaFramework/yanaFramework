...
function database(active, dbms, host, port, user, pass, pass2, name, frame)
{
    if (active == "1" && host > "" && user > "" && frame > "") {
        if (pass != pass2) {
            alert(language['pass_miss']);
        } else {
            var o = document.getElementById(frame);
            o.style.display = 'block';
            o.src = language['php_self']+'?action=database&active=1'+
                '&dbms='+dbms+'&host='+host+'&port='+port+'&user='+user+
                '&pass='+pass+'&name='+name;
        }
    }
}
function ldap(active, host, frame)
{
    if (active == "1" && host > "") {
        var o = document.getElementById(frame);
        o.style.display = 'block';
        o.src = language['php_self']+'?action=ldap&active=1&host='+host;
    }
}
